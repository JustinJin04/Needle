...
## TODO: Implement following by myself
...
class Corpus(object):
    ...
    def tokenize(self, path, max_lines=None):
        ...
        def tokenize_one_line(line: str):
            line_split = line.split()
            for word in line_split:
                ids.append(self.dictionary.add_word(word=word))
            ids.append(eos_id)
        
        with open(path, "r") as f:
            if max_lines is None:
                for line in f:
                    tokenize_one_line(line=line)
            else:
...
def batchify(data, batch_size, device, dtype):
    ...
    data = np.array(data[:nbatch * batch_size]).reshape((batch_size, nbatch)).transpose()
    return data
    ### END YOUR SOLUTION
