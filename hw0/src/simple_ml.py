...
def parse_mnist(image_filename, label_filename):
    ...
    """
    ### BEGIN YOUR CODE
    with gzip.open(image_filename,"rb") as imag_file:
...
def softmax_loss(Z, y):
    ...
    loss=-np.log(softmax_prob[np.arange(Z.shape[0]),y])
    return np.sum(loss)/Z.shape[0]
...
def softmax_regression_epoch(X, y, theta, lr = 0.1, batch=100):
    ...
    # print(X.shape[0],batch)
    Iy=np.eye(theta.shape[1])[y]
    # print(Iy.shape)
    for i in range(0,num_iter):
      X_batch=X[i*batch:(i+1)*batch]
      Iy_batch=Iy[i*batch:(i+1)*batch]
      Z_batch=X_batch @ theta
      # print(X_batch.shape,y_batch.shape)
      exp_Z_batch=np.exp(Z_batch)
      softmax_prob=exp_Z_batch/np.sum(exp_Z_batch,axis=1,keepdims=True)
      grad_theta=(X_batch.T @ (softmax_prob-Iy_batch))/batch
      # print(grad_theta.shape,theta.shape)
      theta-=lr*grad_theta
...
def nn_epoch(X, y, W1, W2, lr = 0.1, batch=100):
    ...
    for i in range(0,num_examples,batch):
      ...
      delta_z2=a2-Iy_batch
      grad_W2=(a1.T @ delta_z2)/batch
      delta_z1=(delta_z2 @ W2.T)*((z1>0).astype(int))
      grad_W1=(X_batch.T @ delta_z1)/batch
